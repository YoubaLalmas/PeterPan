...
export function Parts() {
  ...
  const [spare_parts, setParts] = useState([]);
  const [formValid, setFormValid] = useState(true);

  useEffect(() => {
    fetch("http://localhost:3002/spare_parts")
  ...
  const filteredSpare_parts = !query
    ? spare_parts
    : spare_parts.filter((spare_part) =>
        spare_part.SpareName.toLowerCase()
          .replaceAll(" ", "")
  ...
  return (
    <div className="w-3/4 md:w-2/3 mx-auto p-12">
      <SearchToolForm
        ...
        placeholder={" Enter your Spare Part Name "}
      />
      <div className="relative overflow-x-auto pt-5">
        <table className="w-full text-sm text-left text-gray-600 table-auto">
          ...
          <tbody>
            {filteredSpare_parts?.map((spare_part) => (
              <tr key={spare_part.id} className="bg-white border-b">
                <td className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">
                  {spare_part.SpareCode}
                </td>
                <td className="px-6 py-4">{spare_part.SpareType}</td>
                <td className="px-6 py-4">{spare_part.SpareName}</td>
                <td className="px-6 py-4">{spare_part.Quantity}</td>
                <td className="px-6 py-4">{spare_part.SupplierID}</td>
        ...
        {isOpen && <PartModal onClose={() => setIsOpen(false)} />}
      </div>
    </div>
  );
}
const PartModal = ({ onClose }) => {
  const handleSubmit = (event) => {
    event.preventDefault();
    const formData = new FormData(event.target);
    const partData = {};

    // Populate partData object with form values
    formData.forEach((value, key) => {
      partData[key] = value;
    });
