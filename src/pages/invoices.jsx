    ...
    fetch("http://localhost:3002/invoices")
      ...
      .then((data) => {
        ...
        const uniqueClientIDs = [
          ...new Set(data.map((invoice) => invoice.ClientID)),
        ];

        uniqueClientIDs.forEach((clientID) => {
          fetch(`http://localhost:3002/clients/${clientID}`)
            .then((response) => response.json())
            .then((client) => {
...
const InvoiceModal = ({ onClose }) => {
  const handleSubmit = (event) => {
    ...
    // Populate invoiceData object with form values
    formData.forEach((value, key) => {
      invoiceData[key] = value;
    });
