...
export function Tools() {
  ...
  useEffect(() => {
    fetch("http://localhost:3002/equipments")
  ...
  return (
    <div className="w-3/4 md:w-2/3 mx-auto p-12">
      <SearchToolForm title={"Equipments List"} buttonTitle={"Search"} />
      <div className="relative overflow-x-auto pt-5">
        <table className="w-full text-sm text-left text-gray-600">
          ...
          <tbody>
            {equipments.map((equipment) => (
              <tr key={equipment.id} className="bg-white border-b">
                <td className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">
                  {equipment.SerialNumber}
                </td>
                <td className="px-6 py-4">{equipment.EquipmentName}</td>
                <td className="px-6 py-4">{equipment.EquipmentType}</td>
                <td className="px-6 py-4">{equipment.ClientID}</td>
        ...
        {isOpen && <EquipmentModal onClose={() => setIsOpen(false)} />}
      </div>
    </div>
  );
}
...
const EquipmentModal = ({ onClose }) => {
  const handleSubmit = (event) => {
    ...
    // Populate equipmentData object with form values
    formData.forEach((value, key) => {
      equipmentData[key] = value;
    });
